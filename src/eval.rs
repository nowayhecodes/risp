...
fn builtin_op(mut v: &mut Lval, func: &str) -> RispResult {
    let mut child_count;
    match *v {
        ...
        match func {
            ...
            "/" | "div" => {
                if y.as_num()? == 0 {
                    ...
                    return Err(RispError::DivideByZero);
                } else {
...
pub fn builtin_add(a: &mut Lval) -> RispResult {
    builtin_op(a, "+")
}

pub fn builtin_sub(a: &mut Lval) -> RispResult {
    builtin_op(a, "-")
}

pub fn builtin_mul(a: &mut Lval) -> RispResult {
    builtin_op(a, "*")
}

pub fn builtin_div(a: &mut Lval) -> RispResult {
    builtin_op(a, "/")
}

pub fn builtin_pow(a: &mut Lval) -> RispResult {
    builtin_op(a, "^")
}

pub fn builtin_rem(a: &mut Lval) -> RispResult {
    builtin_op(a, "%")
}

pub fn builtin_max(a: &mut Lval) -> RispResult {
    builtin_op(a, "max")
}

pub fn builtin_min(a: &mut Lval) -> RispResult {
    builtin_op(a, "min")
}
...
fn builtin_var(e: &mut Lenv, a: &mut Lval, func: &str) -> RispResult {
    ...
    match *args {
        Lval::Qexpr(names) => {
            ...
            if vals_len != names_len {
                Err(RispError::NumArguments(names_len, vals_len))
            } else {
...
// BROKEN
//pub fn builtin_def_stub(_v: &Lval) -> RispResult {
//    Ok(lval_sexpr())
//}

// FOR NOW def IS LOCAL ENV ASSIGN
fn builtin_def(e: &mut Lenv, v: &mut Lval) -> RispResult {
    builtin_var(e, v, "def")
}

pub fn builtin_put_stub(_v: &mut Lval) -> RispResult {
    Ok(lval_sexpr())
}

//BROKEN
//fn builtin_put(e: &mut Lenv, v: &Lval) -> RispResult {
//    builtin_var(e, v, "=")
//}

// Attach a value to the front of a qexpr
pub fn builtin_cons(v: &mut Lval) -> RispResult {
    let child_count = v.len()?;
    if child_count != 2 {
        return Err(RispError::NumArguments(2, child_count));
    }
...
pub fn builtin_eval_stub(_v: &mut Lval) -> RispResult {
    Ok(lval_sexpr())
}

// Evaluate qexpr as a sexpr
pub fn builtin_eval(e: &mut Lenv, v: &mut Lval) -> RispResult {
    let qexpr = lval_pop(v, 0)?;
    match *qexpr {
        Lval::Qexpr(ref children) => {
...
pub fn builtin_exit(_v: &mut Lval) -> RispResult {
    // always succeeds
    println!("Goodbye!");
    ::std::process::exit(0);
}
...
pub fn builtin_head(v: &mut Lval) -> RispResult {
...
pub fn builtin_init(v: &mut Lval) -> RispResult {
    let qexpr = lval_pop(v, 0)?;
    match *qexpr {
        Lval::Qexpr(ref children) => {
            let mut ret = lval_qexpr();
...
pub fn builtin_join(v: &mut Lval) -> RispResult {
    let mut ret = lval_qexpr();
    for _ in 0..v.len()? {
        ...
        match *next {
            ...
            _ => {
                return Err(RispError::WrongType(
                    "qexpr".to_string(),
                    format!("{:?}", next),
                ))
            }
...
pub fn builtin_lambda(v: &mut Lval) -> RispResult {
...
pub fn builtin_list(v: &mut Lval) -> RispResult {
    match *v {
        Lval::Sexpr(ref children) => {
...
pub fn builtin_len(v: &mut Lval) -> RispResult {
    let child_count = v.len()?;
    match child_count {
        1 => {
            let qexpr = lval_pop(v, 0)?;
        ...
        _ => Err(RispError::NumArguments(1, child_count)),
    }
}

pub fn builtin_printenv_stub(_v: &mut Lval) -> RispResult {
    Ok(lval_sexpr())
}

// Print all the named variables in the environment
pub fn builtin_printenv(e: &mut Lenv) -> RispResult {
    // we don't use the input
    lval_eval(e, &mut *e.list_all()?)
}

pub fn builtin_tail(v: &mut Lval) -> RispResult {
    let mut qexpr = lval_pop(v, 0)?;
...
pub fn lval_call(e: &mut Lenv, f: Lval, args: &mut Lval) -> RispResult {
    match f {
        Lval::Fun(func) => {
            match func {
                ...
                LvalFun::Lambda(env, mut formals, body) => {
                    ...
                    while args.len()? > 0 {
                        ...
                        if formals.len()? == 0 {
                            return Err(RispError::NumArguments(total, given));
                        }
                        ...
                        if &sym.as_string()? == "&" {
                            ...
                            if formals.len()? != 1 {
                                return Err(RispError::FunctionFormat);
                            }
...
fn eval_cells(e: &mut Lenv, cells: &[Box<Lval>]) -> RispResult {
...
pub fn lval_eval(e: &mut Lenv, v: &mut Lval) -> RispResult {
    let child_count;
    let mut args_eval;
    match v {
        Lval::Blispr(forms) => {
